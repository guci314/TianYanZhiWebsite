...
### 依赖插件（可选）
- **PDF.js Viewer**：提供最佳PDF显示效果（强烈推荐）
...
为获得最佳PDF显示效果，请安装PDF.js Viewer插件：

1. **自动安装**
   ```
   WordPress后台 → 插件 → 安装插件 → 搜索 "PDF.js Viewer"
   ```

2. **手动下载**
   ```
   下载地址：https://wordpress.org/plugins/pdfjs-viewer-shortcode/
   ```
