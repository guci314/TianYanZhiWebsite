...
// 添加管理菜单
add_action('admin_menu', 'dcmp_add_admin_menu', 10);
function dcmp_add_admin_menu() {
    // 添加设置页面到"设置"菜单下
    $settings_page = add_options_page(
        'DC Media Protect 设置',    // 页面标题
        'DC Media Protect',         // 菜单标题
        'manage_options',           // 权限
        'dc-media-protect',         // 菜单slug
        'dcmp_settings_page'        // 回调函数
    );
    ...
    $tools_page = add_management_page(
        ...
        '内容采集',                 // 菜单标题
        'manage_options',           // 权限
        'dcmp-content-crawler',     // 菜单slug
        'dcmp_content_crawler_page' // 回调函数
    );
    
    // 调试信息（仅在调试模式下显示）
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('DC Media Protect 菜单注册结果:');
        error_log('设置页面: ' . ($settings_page ? '成功 (' . $settings_page . ')' : '失败'));
        error_log('工具页面: ' . ($tools_page ? '成功 (' . $tools_page . ')' : '失败'));
    }
}

// 添加管理通知，显示菜单状态
add_action('admin_notices', 'dcmp_menu_debug_notice');
function dcmp_menu_debug_notice() {
    // 只在插件相关页面显示
    $screen = get_current_screen();
    if (isset($_GET['dcmp_menu_debug']) && current_user_can('manage_options')) {
        global $submenu;
        
        echo '<div class="notice notice-info is-dismissible">';
        echo '<h3>DC Media Protect 菜单调试信息</h3>';
        
        // 检查设置菜单
        $settings_exists = false;
        if (isset($submenu['options-general.php'])) {
            foreach ($submenu['options-general.php'] as $item) {
                if ($item[2] === 'dc-media-protect') {
        ...
        if (isset($submenu['tools.php'])) {
            foreach ($submenu['tools.php'] as $item) {
                if ($item[2] === 'dcmp-content-crawler') {
                    $tools_exists = true;
                    break;
                }
            }
        ...
        echo '<p><strong>设置菜单:</strong> ' . ($settings_exists ? '✅ 已注册' : '❌ 未找到') . '</p>';
        echo '<p><strong>工具菜单:</strong> ' . ($tools_exists ? '✅ 已注册' : '❌ 未找到') . '</p>';
        
        if ($settings_exists) {
            echo '<p><a href="' . admin_url('options-general.php?page=dc-media-protect') . '" class="button">→ 访问设置页面</a></p>';
        }
        if ($tools_exists) {
            echo '<p><a href="' . admin_url('tools.php?page=dcmp-content-crawler') . '" class="button">→ 访问内容采集页面</a></p>';
        }
...
function dcmp_settings_page() {
    ...
    <div class="wrap">
        ...
        <div style="margin-top: 30px; padding: 15px; background: #f0f8ff; border: 1px solid #0073aa; border-radius: 5px;">
            <h3>功能快捷链接</h3>
            <p><a href="<?php echo admin_url('tools.php?page=dcmp-content-crawler'); ?>" class="button">📥 内容采集</a></p>
            <p><a href="<?php echo admin_url('tools.php?page=dcmp-content-crawler'); ?>" class="button">✏️ 手动导入</a></p>
        </div>
    </div>
